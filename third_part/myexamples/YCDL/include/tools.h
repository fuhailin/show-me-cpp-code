...
template <class T>
void line_split(const std::string &line, const std::string &sep,
                std::vector<T> &ans) {
...
struct RandomDouble {
  ...
  double get_random() { return unif(re); }
...
std::vector<double> calc(std::vector<std::pair<int, double>> &label_pre,
                         double threshord = 0.5) {
