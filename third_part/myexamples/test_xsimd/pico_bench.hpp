...
class Statistics {
 ...
 public:
  ...
  T percentile(const float p) const { return percentile(p, samples); }

  // Winsorize the data, sets all entries above 100 - limit percentile and
  // below limit percentile to the value of that percentile
  void winsorize(const float limit) { winsorize(limit, samples); }

  T median() const { return percentile(50.0, samples); }
  ...
  T mean() const {
    const auto m =
        std::accumulate(std::begin(samples), std::end(samples), T{0});
  ...
  T std_dev() const {
    ...
    auto val = std::accumulate(
        std::begin(samples), std::end(samples), T{0},
        [&m](const T& p, const T& t) {
          return T{static_cast<rep>(p.count() + std::pow((t - m).count(), 2))};
        });
    return T{
        static_cast<rep>(std::sqrt(1.0 / static_cast<double>(samples.size()) *
                                   static_cast<double>(val.count())))};
  }

  T min() const { return samples.front(); }

  T max() const { return samples.back(); }

  std::size_t size() const { return samples.size(); }

  const T& operator[](size_t i) const { return samples[i]; }
...
class Benchmarker {
 ...
 public:
  ...
  typename std::enable_if<
      std::is_same<decltype(std::declval<Fn>()()), T>::value, stats_type>::type
  operator()(Fn fn) const {
    ...
    std::vector<T> samples;
    for (size_t i = 0;
         i < MAX_ITER && (MAX_RUNTIME.count() == 0 || elapsed < MAX_RUNTIME);
