...
inline void writePPM(const std::string& fileName, const int sizeX,
                     const int sizeY, const int* pixel) {
...
template <class arch>
void mandelbrot(float x0, float y0, float x1, float y1, int width, int height,
                int maxIters, int output[]) {
  using float_batch_type = batch<float, arch>;
  using int_batch_type = batch<int, arch>;
...
void run_arch(bencher_t& bencher, float x0, float y0, float x1, float y1,
              int width, int height, int maxIters,
              std::vector<int, xsimd::aligned_allocator<int>>& buffer) {
  std::fill(buffer.begin(), buffer.end(), 0);
  auto stats = bencher([&]() {
    xsimd::mandelbrot<arch>(x0, y0, x1, y1, width, height, maxIters,
                            buffer.data());
  });
  ...
  std::cout << '\n' << arch::name() << " " << stats << '\n';
  auto filename = std::string("mandelbrot_") + std::string(arch::name()) +
                  std::string(".ppm");
...
struct run_archlist<xsimd::arch_list<Arch...>> {
  ...
  static void run(bencher_t& bencher, float x0, float y0, float x1, float y1,
                  int width, int height, int maxIters,
                  std::vector<int, xsimd::aligned_allocator<int>>& buffer) {
    using expand_type = int[];
    expand_type{(run_arch<Arch>(bencher, x0, y0, x1, x1, width, height,
                                maxIters, buffer),
                 0)...};
  }
};
...
int main() {
  ...
  run_archlist<xsimd::supported_architectures>::run(
      bencher, x0, y0, x1, y1, width, height, maxIters, buf);
