 ...
 * @Date: 2019-08-16 15:15:28
 */
//
// Created by Feng Shen on 04/03/2018.
//
...
class Buffer {
   private:
    size_t size = 2048;
    char b[2048];  // on the stack
    bool in_heap;
    char *p;

   public:
    Buffer() : in_heap(false) { p = b; };
...
class BufferedWriter {
   ...
   public:
    BufferedWriter(int fd, size_t cap) : fd(fd), cap(cap), off(0), buf((char *)malloc(cap)) {}
...
class BufferedReader {
   ...
   public:
    BufferedReader(int fd, size_t cap) : fd(fd), cap(cap), buf((char *)malloc(cap)), read(0), limit(0) {}
...
struct SlaveInfo {
    ...
    SlaveInfo(std::string &host, int port, int fd) : host(host), port(port), fd(fd), seq(0) {}
};
...
struct MasterOptions {
    ...
    MasterOptions(rocksdb::DB *db, std::mutex &mtx, std::condition_variable &cv, BaseConfig *conf)
...
class SequenceManager {
   ...
   public:
    SequenceManager() : seq(-100), fd(-1) {}

    int Open(const std::string &dir, const std::string &host, const int port) {
        rocksdb::Env *env = rocksdb::Env::Default();
        const std::string fname = dir + "/" + host + "-" + std::to_string(port) + ".seq";
        uint64_t size = 0;
        env->GetFileSize(fname, &size);
        fd = open(fname.data(), O_RDWR | O_CREAT, FILE_PERM);
        if (fd < 0) {
            log_error("[slave] open seq file %s, %s", fname.data(), strerror(errno));
            return -1;
        }
        if (size == 8) {
            char buffer[8];
            if (readn(fd, buffer, 8, 8) < 8) {
                log_error("[slave] seq file %s, %s", fname.data(), strerror(errno));
                return -1;
            }
            int64_t buf_tmp;
        ...
        log_info("[slave] read seq %ld from %s", Get(), fname.data());
        return 0;
    }
    ...
    int write(rocksdb::SequenceNumber n) {
        if (pwrite(fd, (char *)(&n), 8, 0) != 8) {
            log_error("[slave] write seq %lu fail, %s", n, strerror(errno));
            return -1;
        }
...
struct SlaveManager {
    ...
    void sync(rocksdb::DB *db) {  // 增量更新
        while (1) {               // never exits
            ...
            while (1) {
                if (br->ReadByte(&flags) < 0 || br->Read64(&seq) < 0 || br->Read64(&latest_seq) ||
                    br->Read32(&length) < 0 || br->Read(&content, length) < 0) {
                    // log_error("[slave] read change, seq %ld, %s", seq, strerror(errno));
                    log_error("[slave] read change, seq  %s", strerror(errno));
                    break;
                }
                ...
                // log_info("[slave] apply change %lld", seq);
                if (!status.ok() || sm.write(seq) < 0) {
                    log_error("[slave] apply batch of seq %ld, %s", seq, status.ToString().data());
                    break;
                }
            ...
            while (1) {
                log_error("[slave] connection from %s:%d broken, seq %ld, sleep %ds", conf->master.ip.data(),
                          conf->master.port, sm.Get(), seconds);
                std::this_thread::sleep_for(std::chrono::seconds(seconds));
                int t = open_client(conf->master.ip, conf->master.port, sm.Get());
                if (t > 0) {
                    log_error("[slave] connection to %s:%d ok, sync from seq %ld", conf->master.ip.data(),
                              conf->master.port, sm.Get());
                    this->fd = t;
   ...
   public:
    ...
    SlaveManager(BaseConfig *conf, std::mutex &mtx, std::condition_variable &cv) : conf(conf), mtx(mtx), cv(cv) {
        master_seq = 0;
        br = nullptr;
    }
    ...
    int Open() {
        if (sm.Open(conf->wal_dir.empty() ? conf->data_dir : conf->wal_dir, conf->master.ip, conf->master.port) < 0)
            return -1;
    ...
    int64_t InitDump() {
        ...
        if (br->Read32(&file_count) < 0) {
            log_error("[slave] read file count, %s", strerror(errno));
            return -1;
        }

        log_info("[slave] begin load %d live files from master", file_count);
        for (int i = 0; i < file_count; i++) {
            // read file_name, content_length
            if (br->Read32(&name_size) < 0 || br->Read(&fnp, name_size) < 0 || br->Read64(&content_length)) {
                log_error("[slave] read %dth file name, %s", i + 1, strerror(errno));
                return -1;
            }
            ...
            if (ffd < 0) {
                log_error("[slave] create %dth file %s, %s", i + 1, fname.data(), strerror(errno));
                return -1;
            }
        ...
        //cout<<"======================================="<<endl<<"=================="<<i<<"=================="<<endl<<"==========================="<<endl;
                ...
                if (br->Read(&p, n) < 0) {
                    ...
                    log_error("[slave] read %dth content, %s", i + 1, strerror(errno));
                    return -1;
                }
        //cout<<"======================================="<<endl<<"=================="<<i<<"=================="<<endl<<"==========================="<<endl;
                ...
                while (nleft > 0) {
                    ...
                    if ((nwritten = write(ffd, p, nleft)) <= 0) {
                        if (errno == EINTR)
                            ...
                            log_error("[slave] write %dth content, %s", i + 1, strerror(errno));
                            return -1;
                        };
                    }
                    nleft -= nwritten;
        ...
        if (br->Read64(&seq) < 0) {
            log_error("[slave] read seq, %s", strerror(errno));
            return -1;
        }

        if (sm.write(seq) >= 0) {
            log_info("[slave] done load %d live files from master, sync from seq %ld", file_count, seq);
            return seq;
        } else {
            log_error("[slave] write seq failed %ld, %s", seq, strerror(errno));
            return -1;
        }
    }
...
class RockServerBase {
   ...
   public:
    ...
    int Open0() {
        ...
        // std::cout<<options.max_write_buffer_number<<std::endl;
        ...
        for (size_t i = 0; i != conf->columnFamilies.size(); i++) {
            column_families.emplace_back(conf->columnFamilies[i], rocksdb::ColumnFamilyOptions(options));
            cfIndex[conf->columnFamilies[i]] = i + 1;
        }
        ...
        if (conf->mode == kModeMaster) {
            status = rocksdb::DB::Open(options, db_dir, column_families, &cfHandles, &db);

            if (!status.ok()) {
                log_error("open db %s, %s", db_dir.data(), status.ToString().data());
                return -1;
            }
        } else {

            sm = new SlaveManager(conf, msMutex_, msCondVar_);
            if (sm->Open() < 0) return -1;

            if (sm->GetSeq() < 0 && sm->InitDump() < 0) {
                log_error("init dump error to dir %s, got error, exit", db_dir.data());
                return -1;
            }
            status = rocksdb::DB::Open(options, db_dir, column_families, &cfHandles, &db);
            if (!status.ok()) {
    ...
    bool Command0(std::vector<std::string> &_return, const std::vector<std::string> &argv) {
        const std::string &command = argv[0];
        if (command == "backup" && argv.size() == 2) {
            rocksdb::Checkpoint *cp = NULL;
	    rocksdb::Checkpoint::Create(db,&cp);
            rocksdb::Status status = cp->CreateCheckpoint(argv[1]);
            _return.push_back(status.ToString());
        } else if (command == "flush") {
            for (auto f : cfHandles) {
                const rocksdb::Status &status = db->Flush(rocksdb::FlushOptions(), f);
                _return.push_back(f->GetName());
                _return.push_back(status.ToString());
            }
        } else if (command == "config" && argv[1] == "set" && argv.size() >= 4) {
            std::unordered_map<std::string, std::string> m;
            if (argv.size() % 2 == 0) {  // set default column family
                for (size_t i = 2; i < argv.size(); i += 2) {
                    m[argv[i]] = argv[i + 1];
                }
                const rocksdb::Status status = db->SetOptions(m);
                _return.emplace_back("column_family:default");
                _return.push_back(status.ToString());
            } else {
                ...
                if (argv[2] == "db") {
                    const rocksdb::Status &status = db->SetDBOptions(m);
                    _return.emplace_back("db");
                    _return.push_back(status.ToString());
                } else if (cfIndex.find(argv[2]) != cfIndex.end()) {
                    rocksdb::ColumnFamilyHandle *h = getCFHandle(argv[2]);
                    const rocksdb::Status &status = db->SetOptions(h, m);
                    _return.emplace_back("column_family:" + argv[2]);
                    _return.push_back(status.ToString());
                } else {
                    _return.emplace_back("unknown");
                    _return.emplace_back("column_family:" + argv[2]);
                }
            }
        } else if (command == "info" && argv.size() >= 2) {
            util::set_log_level(parse_log_level(argv[1]));
    ...
    int Info0(char *buf, size_t limit) {
        int n = snprintf(buf, limit, "[uptime]: %s\n", db->GetEnv()->TimeToString(up_time).data());
        if (sm != nullptr) {
            n += snprintf(buf + n, limit - n, "[slave]: sync seq %ld, master is at %ld\n", sm->GetSeq(),
                          sm->MasterSeq());
        }
        if (m_opt != nullptr) {
            std::vector<SlaveInfo *> slaves;
            {
                std::lock_guard<std::mutex> _(m_opt->l_mtx);
                slaves = m_opt->slaves;
            }
            if (!slaves.empty()) {
                n += snprintf(buf + n, limit - n, "[master]: %lu slave(s) connected, master is at seq %lu\n",
                              slaves.size(), db->GetLatestSequenceNumber());
                for (size_t i = 0; i < slaves.size(); i++) {
                    const auto &s = slaves[i];
                    n += snprintf(buf + n, limit - n, "          %lu) %s:%d, fd %d, at seq %ld\n", i + 1,
                                  s->host.data(), s->port, s->fd, s->seq.load(std::memory_order_acquire));
                }
            }
        }
        return n;
    ...
    virtual void MoreOptions(rocksdb::Options &options) = 0;
};
