...
void Logger::Logv(int level, const char *fmt, va_list ap) {
    ...
    int _n = snprintf(_out, sizeof(msg), "%s %c [t%d] %s\n", buf, levels[level], thread_id_, msg);
    {
        ...
        if (this->reopen_) {  // reopen
            this->reopen_ = false;
            this->Close();
            if (!this->filename_.empty() && this->filename_ != "stdout") logfile_ = fopen(this->filename_.c_str(), "a");
        }
        if (!thread_id_) {
...
bool log_open(const std::string &filename, int level) { return g_logger.Open(filename, level); }
