...
// void MyOp::push(const std::vector<Key> & keys, const std::vector<vector<double>> & Vals)
void MyOp::push(const std::map<Key, std::string> & key_vals, bool is_sync)
{
    ...
    for (auto it = key_vals.begin(); it != key_vals.end(); ++it) {
        ...
        if (opti_ == "adagrad" || opti_ == "rmsp"){    
            status = db->Get(rocksdb::ReadOptions(), cfHandles.at("ada"), std::to_string(fid), &values);
            assert(status.ok());
        ...
        status = db->Get(rocksdb::ReadOptions(), cfHandles.at("emb"), std::to_string(fid), &values);
        assert(status.ok());
        ...
        if(opti_ == "adagrad") 
        {
            ...
            para_dict -= learn* grad.cwiseProduct((ada_sum+MatrixXf::Constant(1, val_size, 1E-6)).cwiseSqrt().cwiseInverse());
        }else if(opti_ == "rmsp")
        {
            ada_sum = conf->rmsp_beta*ada_sum + grad.cwiseAbs2();
            para_dict -= learn* grad.cwiseProduct((ada_sum.cwiseSqrt()+MatrixXf::Constant(1, val_size, 1E-7)).cwiseInverse());
        }else
        {
        ...
        // std::cout<<"++++++++++the emb+++++++++++"<<std::endl<<para_dict<<std::endl;
        
        int64_t latest_time = (int64_t)std::chrono::duration_cast<std::chrono::duration<double>>(
                    std::chrono::system_clock::now().time_since_epoch()).count();
        
        status = db->Get(rocksdb::ReadOptions(), cfHandles.at("update_times"), std::to_string(fid), &values);
        ...
        batch.Put(cfHandles.at("update_times"), std::to_string(fid), std::to_string(up_tmp));
        batch.Put(cfHandles.at("latest_time"), std::to_string(fid), std::to_string(latest_time));
    }
    // exit(true);
    if (batch.Count()) {
        ...
        //=====================================
        // std::cout<<"==========before write==========="<<std::endl<<log_Time()<<std::endl;
        db->Write(write_options, &batch);
        // std::cout<<log_Time()<<std::endl<<"==============end write============"<<std::endl;
...
// void MyOp::pushemb(const std::vector<Key> & keys, const std::vector<vector<double>> & Vals)
...
// void MyOp::pull(const std::vector<Key> & keys, std::vector<std::string> & Vals)
void MyOp::pull(std::map<Key, std::string>& key_vals, const std::set<Key>& keys)
{
    ...
    for (auto key:keys) {
        ...
        std::string values;
        // string tmp()
        rocksdb::Status state = db->Get(rocksdb::ReadOptions(), cfHandles.at("emb"), std::to_string(key), &values);
        ...
        if(state.IsNotFound())
        {
            ...
            try {
                ...
                std::cout<<"The slot "<<slot<<"has no feature in the meta file"<<std::endl;
                assert(false);
            }
            ...
            if (opti_ == "adagrad" || opti_ == "rmsp"){    
                batch.Put(cfHandles.at("ada"), std::to_string(key), vec2str(MatrixXf::Constant(1, _length, ada_beta)));
            }
            batch.Put(cfHandles.at("latest_time"), std::to_string(key), std::to_string(latest_time));
...
void MyOp::multi_pull(std::map<Key, std::string>& key_vals, const std::set<Key>& keys) {
    rocksdb::WriteBatch batch;
    ...
    for (auto s:keys) {
        ...
        tmp_handle.push_back(cfHandles.at("emb"));
                    ...
                    std::chrono::system_clock::now().time_since_epoch()).count();
    if (end-start>0.5) std::cout<<"The first loop takes "<<end-start<<"s"<<std::endl;

    // std::cout<<tmp_handle.size()<<std::endl;
    std::vector<std::string> values;
    start = (double)std::chrono::duration_cast<std::chrono::duration<double>>(
                    std::chrono::system_clock::now().time_since_epoch()).count();
    std::vector<rocksdb::Status> status = db->MultiGet(rocksdb::ReadOptions(), tmp_handle, slices, &values);
    end = (double)std::chrono::duration_cast<std::chrono::duration<double>>(
                    std::chrono::system_clock::now().time_since_epoch()).count();
    if (end-start>0.5) std::cout<<"The multiget takes "<<end-start<<"s"<<std::endl;                
    ...
    for (size_t i(0); i<status.size(); ++i) {
        if (status.at(i).IsNotFound()) {
            ...
            if (opti_ == "adagrad" || opti_ == "rmsp"){    
                batch.Put(cfHandles.at("ada"), keys_str.at(i), vec2str(MatrixXf::Constant(1, _length, ada_beta)));
            }
            batch.Put(cfHandles.at("latest_time"), keys_str.at(i), std::to_string(latest_time));
            ...
            key_vals.insert(std::make_pair(std::stoll(keys_str.at(i)), values.at(i)));
                    ...
                    std::chrono::system_clock::now().time_since_epoch()).count();
    if (end-start>0.5) std::cout<<"The second loop takes "<<end-start<<"s"<<std::endl;
    ...
    if(batch.Count())
    {
        ...
        rep_b->notify_replication();
                    ...
                    std::chrono::system_clock::now().time_since_epoch()).count();
    if (end-start>0.5) std::cout<<"The write takes "<<end-start<<"s"<<std::endl;
...
// void MyOp::pull_online(const std::vector<Key> & keys, std::vector<std::string> & Vals)
void MyOp::pull_online(std::map<Key, std::string> & key_vals, const std::set<Key>& keys)
{
    ...
    start = (double)std::chrono::duration_cast<std::chrono::duration<double>>(
                    std::chrono::system_clock::now().time_since_epoch()).count();
    std::vector<rocksdb::Status> status_emb = db->MultiGet(rocksdb::ReadOptions(), emb_handle, slices, &values_emb);
    std::vector<rocksdb::Status> status_update = db->MultiGet(rocksdb::ReadOptions(), update_handle, slices, &values_update);
    end = (double)std::chrono::duration_cast<std::chrono::duration<double>>(
                    std::chrono::system_clock::now().time_since_epoch()).count();
    if (end-start>0.5) std::cout<<"The multiget takes "<<end-start<<"s"<<std::endl;                
    // std::cout<<"haha"<<std::endl;
    ...
    for (size_t i(0); i<status_emb.size(); ++i) {
        if (status_emb.at(i).IsNotFound()) {
            ...
            if (update_times_real > update_std) {
                key_vals.insert(std::make_pair(std::stoll(keys_str.at(i)), values_emb.at(i)));
            } else {
                int& _length = fea_use_length.at(slot);
                MatrixXf tmp = MatrixXf::Zero(1, _length);
                key_vals.insert(std::make_pair(std::stoll(keys_str.at(i)), vec2str(tmp)));
            }
        }
    }
    end = (double)std::chrono::duration_cast<std::chrono::duration<double>>(
