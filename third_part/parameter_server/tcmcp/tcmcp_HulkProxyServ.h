...
#include "thrift/HulkProxyThrift/HulkProxyServ.h",
// diy include <other.h> if you need.

class tcmcp_HulkProxyServClient
    : public CThriftClinetMultiConnectPlug<hulk::HulkProxyServClient> {
 public:
  /*
   * constructor
   */
  tcmcp_HulkProxyServClient(const string zkHost, const string zkPath,
                            const int connectNumber,
                            const int timeout = 1000 * 10,
                            const double checkTimes = 1.0)
      : CThriftClinetMultiConnectPlug<hulk::HulkProxyServClient>(
 ...
 public:
  ...
  int tcmcp_hmpush(const std::string& hash,
  ...
  int tcmcp_hpush(const std::string& hash, const std::string& key,
                  const std::string& value);

  int tcmcp_get(std::string& _return, const std::string& key);

  int tcmcp_hget(std::string& _return, const std::string& hash,
                 const std::string& key);

  int tcmcp_mget(std::map<std::string, std::string>& _return,
  ...
  int tcmcp_hmget(std::map<std::string, std::string>& _return,
                  const std::string& hash, const std::set<std::string>& keys);

  int tcmcp_hgetall(std::map<std::string, std::string>& _return,
                    const std::string& hash);

  int tcmcp_hgetall_limit(std::map<std::string, std::string>& _return,
                          const std::string& hash, const int32_t limit);

  int tcmcp_hscan(::hulk::ScanData& _return, const std::string& hash,
                  const std::string& cursor, const int32_t count);
