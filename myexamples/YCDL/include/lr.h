...
public:
  ...
  LRmodel(std::shared_ptr<Optimizer> &sparse_weight_sgd,
          std::shared_ptr<loss_func_layer> &loss_layer, int len = 1) {
    _dim = len;
  ...
  void forward(std::vector<Instance> &instances, bool is_train = true) {
    for (int j = 0; j < instances.size(); j++) {
      ...
      std::vector<std::vector<double>> weight(ins.feas.size(),
  ...
  void backward(std::vector<Instance> &instances) {
    for (int j = 0; j < instances.size(); j++) {
      ...
      std::vector<std::vector<double>> grads(ins.feas.size(),
                                             std::vector<double>(_dim, 0.0));
      for (int k = 0; k < ins.feas.size(); k++) {
        for (int l = 0; l < _dim; l++) {
  ...
  void stat(std::vector<Instance> &instances) {
    ...
    LOG(INFO) << "auc: " << calc_auc(label_pre) << ", loss: " << loss
              << ", acc: " << out[0] << ", pre: " << out[1]
              << ", recall: " << out[2] << ", pre_avg: " << pre_avg
              << ", label_avg: " << label_avg;
  }
};
} // namespace YCDL
