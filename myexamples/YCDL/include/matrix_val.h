...
class MatrixValue {
 public:
  ...
  virtual void pull(std::vector<std::vector<SLOT_ID_FEAS>> &lines,
                    bool is_train = true) {}
  ...
  virtual void init(std::string name, int row, int col,
                    std::shared_ptr<Optimizer> opt, bool need_gradient) {}

  virtual void init(std::vector<int> slot_vec, int dim,
                    std::shared_ptr<Optimizer> opt, bool need_gradient) {}
 ...
 public:
  ...
  void pull(bool is_train = true) override {
    ...
    std::vector<std::vector<double>> vals(
        1, std::vector<double>(_row * _col, 0.0));
...
class SparseMatrixValue : public MatrixValue {
 public:
  void init(std::vector<int> slot_vec, int dim, std::shared_ptr<Optimizer> opt,
            bool trainable) override {
    _slot_vec = slot_vec;
    _dim = dim;
    _optimizer = opt;
  ...
  void pull(std::vector<std::vector<SLOT_ID_FEAS>> &lines,
    ...
    for (auto &line : lines) {
      for (auto &slot_pair : line) {
        ...
        for (auto fea_str : feas_str) {
          std::string slot_fea =
              boost::lexical_cast<std::string>(slot_id) + ":" + fea_str;
    ...
    // pull feas
    std::vector<std::vector<double>> vals(feas.size(),
                                          std::vector<double>(_dim, 0.0));
...
class MatrixValueMap {
 public:
  ...
  std::unordered_map<std::string, std::shared_ptr<MatrixValue>>
      _matrix_value_map;
};
