...
DEFINE_string(filename,
              "/root/projects/third_part/myexamples/test_tensorflow/tf2/data/"
              "test1.tfrecord",
              "filename");
...
static tensorflow::Status ParseExample(const tensorflow::Scope& scope,
  ...
  for (int i = 0; i < num_keys; ++i) {
    ...
    key.scalar<tensorflow::tstring>()() =
        tensorflow::strings::Printf("feature_%d", i);
...
int main(int argc, char* argv[]) {
  ...
  tensorflow::Tensor filenames(FLAGS_filename);  // Check tfrecord_test.py which
