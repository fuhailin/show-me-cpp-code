...
bool test_iostream_roundtrip(CompressorArgs&&... args) {
  std::string original =
      "hello, world! this test verifies that iostreams can "
...
int main() {
  {
    ...
    std::string expected(
        "\x1f\x8b\x08\x00\x00\x00\x00\x00\x04\xff\xcb\x48\xcd\xc9\xc9\x07\x00"
        "\x86\xa6\x10\x36\x05\x00\x00\x00",
        25);
  ...
  if (!test_iostream_roundtrip<gzip_compressor, gzip_decompressor>(
          zlib::best_speed))
    return 1;
  if (!test_iostream_roundtrip<zlib_compressor, zlib_decompressor>(
          zlib::best_speed))
    return 1;
  if (!test_iostream_roundtrip<zstd_compressor, zstd_decompressor>(
          zstd::best_speed))
    return 1;
  if (!test_iostream_roundtrip<lzma_compressor, lzma_decompressor>(
          lzma::best_speed))
    return 1;
