...
class tcmcp_psProxyServClient:public CThriftClinetMultiConnectPlug<psProxyServClient>
{
...
public:
    ...
    * constructor
    */
    tcmcp_psProxyServClient(const string zkHost, const string zkPath, const int connectNumber, const int timeout=1000*10, const double checkTimes=1.0):
                    CThriftClinetMultiConnectPlug<psProxyServClient>(zkHost, zkPath, connectNumber, timeout, checkTimes){}
...
private:
    ...
    */
    void createConnect(map<STR_ip_port_suffix, connectNode<psProxyServClient>>& pool, const string& ip, const int port);
...
public:
    ...
    */
    ...
    int tcmcp_pull(std::map<Key, std::string> & _return, const std::set<Key> & keys);

    int tcmcp_pull_online(std::map<Key, std::string> & _return, const std::set<Key> & keys);
