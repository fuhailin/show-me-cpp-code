...
void DataSet::ReadCSVFile(string dir, string file_name) {
  ...
  // - binary
  // - file.csv
  // if bazel is used, the csv file will be under some directories relative to
  // the binary
  // - binary
  ...
  if (!file) {
    cerr << "ERROR: No " << file_name << " next to the binary or at " << dir
         << ", please double check the location of the CSV dataset file."
         << endl;
    cerr << "ERROR: The dir option must be relative to the binary location."
         << endl;
  }
  stringstream buffer;
  buffer << file.rdbuf();
...
float DataSet::output(float price) {
  return price * (data_set_metadata.max_price - data_set_metadata.min_price) +
         data_set_metadata.min_price;
}
