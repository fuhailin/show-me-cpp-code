...
typedef ::apache::thrift::stdcxx::shared_ptr<TTransport> atsstt;// 取每个namespace的第一个字母胡乱命名
...
{
    shared_ptr<RWLock> sptr_rwmtx_connectPool;// 互斥量, mutex拷贝、=运算符都是delete，放在标准容器中必须这么用
...
public:
    ...
    */
    CThriftClinetMultiConnectPlug(const string zkHost, const string zkPath, const int connectNumber, const int timeout=1000*10, const double checkTimes=1.0):
        m_zkHost(zkHost), 
        m_zkPath(zkPath),
...
protected:
    ...
    */
    virtual void createConnect(map<STR_ip_port_suffix, connectNode<T>>& pool, const string& ip, const int port) = 0;
...
protected:
    ...
    map<STR_ip_port, time_t> m_blacklist;// 黑名单，BALCKLIST_TIME 秒之内不尝试重连
};
    ...
    else
    {
        ...
        // ReadGuard Zone
        {
            ...
            else
            {
                ...
                it->second.connect_step = it->second.connect_step > 12345 ? 0 : it->second.connect_step + 1;
#ifdef PRINT_INFO
...
template <class T>
bool CThriftClinetMultiConnectPlug<T>::deleteClient(STR_ip_port_suffix connectNodeKey)
{
    ...
    boost::split(ip_port_suffix, connectNodeKey, boost::is_any_of("_"), boost::token_compress_on);
        ...
        && m_connectPool[ip_port_suffix[0]].status != POOL_OFFLINE)
    {
        ...
        // 连接节点存在并且不是下线状态
        if(m_connectPool[ip_port_suffix[0]].pool.find(connectNodeKey) != m_connectPool[ip_port_suffix[0]].pool.end()
            && *(m_connectPool[ip_port_suffix[0]].pool.find(connectNodeKey)->second.status) != CONNECT_OFFLINE
            // 这种写法报错，不知道原因。。
            //m_connectPool[ip_port_suffix[0]].pool[connectNodeKey];
        )
        {
            m_connectPool[ip_port_suffix[0]].pool.find(connectNodeKey)->second.transport->close();
...
// 获取zk信息
template <class T>
void CThriftClinetMultiConnectPlug<T>::getZKInfo(vector<string>& vec_zk_info, bool watch)
{
    vector<string> vecChildNode;
    m_zkClinet.GetChildren(m_zkPath, &vecChildNode, 
                        watch ? watchGetChildren : nullptr, 
                        watch ? static_cast<void*>(this) : nullptr, 
                        watch);
...
template <class T>
void CThriftClinetMultiConnectPlug<T>::addServerInfo(vector<string>* input_ip_port_weight)

{   
    vector<string> *p2zkInfo = nullptr;
    vector<string> vec_zk_info;// 初始化时有用，回调时无用，用以减少回调时的数据拷贝，提高性能
    ...
    for(auto it : *p2zkInfo)
    {
        ...
        try{
            ...
            std::cout<<"Error msg: parse layers failed in addServerInfo function!"<<std::endl;
            continue;
        }
        const string ip = jZkInfo["ip"];
        const int port = jZkInfo["port"];
        const string ip_port = ip + ":" + to_string(port);
        if(m_serverInfo.find(ip_port) == m_serverInfo.end() && !inBlacklist(ip, port))
        {
            ...
            if(connectPool.size() > 0)
            {
                ...
                m_connectPool.insert(pair<STR_ip_port, connectPoolNode<T>>(ip_port, connectPoolNode<T>(move(connectPool))));
#ifdef PRINT_ADD_DELETE_INFO
...
template <class T>
void CThriftClinetMultiConnectPlug<T>::deleteServerInfo(vector<string>& ip_port_weight)
{   
    /*
    * 暂时不感知zk权重信息变化。
    */
    ...
    for(auto it : ip_port_weight)
    {
        ...
        try{
            ...
            std::cout<<"Error msg: parse layers failed in deleteServerInfo function!"<<std::endl;
            continue;
        }
        const string ip = jZkInfo["ip"];
        const int port = jZkInfo["port"];
...
void CThriftClinetMultiConnectPlug<T>::watchZK()
{
    ...
    {   
        ...
        if(m_threadRun)
            usleep(static_cast<int>(1000 * 1000 / m_checkTimes));
    }
}

template <class T>
bool CThriftClinetMultiConnectPlug<T>::inBlacklist(const string& ip, const int port) 
{
    bool ret = true;
...
*/
template <class T>
void CThriftClinetMultiConnectPlug<T>::watchGetChildren(ZKErrorCode errcode, const std::string& path, int count, char** data, void* context)
{

    CThriftClinetMultiConnectPlug* p2this = static_cast<CThriftClinetMultiConnectPlug*>(context);
...
int CThriftClinetMultiConnectPlug<T>::init()
{
    // 初始化zk连接
    {
    if (!m_zkClinet.Init(m_zkHost.c_str(), 1000*60))
        return ERR_INIT_INIT;
    }
...
void CThriftClinetMultiConnectPlug<T>::addClient2ConnectPool()
{
    ...
    for(auto& it : m_connectPool)
    {
        if(it.second.pool.size() < m_connectNumber && it.second.status != POOL_OFFLINE)
        {
            WriteGuard r(*(it.second.sptr_rwmtx_connectPool));
            int number = m_connectNumber - it.second.pool.size();//尝试创建client个数
            vector<string> ip_port;
            boost::split(ip_port, it.first, boost::is_any_of(":"), boost::token_compress_on);
...
template <class T>
string CThriftClinetMultiConnectPlug<T>::getConnectPoolKey(const string& ip_port)
{
...
#define CREATE_CONNECT_TFramedTransport_TBinaryProtocol(__CLIENT_TYPE__)\
    ...
    pool.insert(pair<STR_ip_port_suffix, connectNode<__CLIENT_TYPE__>>(connect.connectNodeKey, move(connect)))\
